function [params, bg_area, fg_area, area_resize_factor] = initializeAllAreas(im, target_size, params)

	% we want a regular frame surrounding the object
	avg_dim = sum(target_size)/2;
	% size from which we extract features
	bg_area = round(target_size + avg_dim);
	% pick a "safe" region smaller than bbox to avoid mislabeling
	fg_area = round(target_size - avg_dim * params.inner_padding);
	...
	bg_area = bg_area - mod(bg_area - target_size, 2);
...
%    norm_target_sz_w = target_size(2) * params.norm_bg_area(2) / bg_area(2);
%	norm_target_sz_h = target_size(1) * params.norm_bg_area(1) / bg_area(1);

...
% Calculate robustness score
   ...
   AveOP = sum(OverlapScore, 2)/expertNum;            % the average overlap 
   expertAveOP = sum(OverlapScore, 1)/period;         % the average performance for each tracker in a period
   VarOP = sqrt( sum((OverlapScore - repmat(expertAveOP, period ,1) ).^2, 2)/expertNum );   % the variance
   weight_sum = sum(weight);
   norm_factor = 1/weight_sum;

...
% TODO: im, bg_area, fg_area, area_resize_factor
    p.grayscale_sequence = false;  % suppose that sequence is colour
    p.hog_cell_size = 4;
    p.fixed_area = 150^2;           % standard area to which we resize the target
    p.n_bins = 2^5;                            % number of bins for the color histograms (bg and fg models)
    p.learning_rate_pwp = 0.04;           % bg and fg color models learning rate 
    p.feature_type = 'fhog';            % {'fhog','gray'}
    p.inner_padding = 0.2;             % defines inner area used to sample colors from the foreground
    p.output_sigma_factor = 1/16 ;             % standard deviation for the desired translation filter output
    p.lambda = 1e-3;                                   % regularization weight
    p.learning_rate_cf = 0.01;            % HOG model learning rate
    p.merge_factor = 0.3;              % fixed interpolation factor - how to linearly combine the two responses
    ...
    target_sz = p.targetSize;
    im = p.imgFiles{p.startFrame};
    if(size(im, 3)==1)
		p.grayscale_sequence = true;
    end    

    [p, bg_area, fg_area, area_resize_factor] = initializeAllAreas(im, target_sz, p);

    num_frames = numel(p.imgFiles);

    bboxes = zeros(num_frames, 4);
    ...
    %% MAIN LOOP
    tic;
    for frame = p.startFrame:num_frames
        ...
        if ~isempty(p.track_lost) && p.track_lost(frame, bboxes(frame,:)),
            break
        end        
    end
    elapsed_time = toc;
    speed = num_frames/elapsed_time;

    n_frames_ontrack = sum(sum(bboxes==0,2)~=4);
    if isempty(p.track_lost)
        speed = (num_frames-p.startFrame+1) / elapsed_time;
    else
        speed = n_frames_ontrack/elapsed_time;
    end    
end
